...
test.describe("3. readFileAndCount", () => {
  ...
  test.test("3. readFileAndCount", (t, done) => {
    ...
    const text =
      "node is a powerful runtime environment that allows you to build scalable and high-performance applications. With node, you can use JavaScript on the server-side, and it has a vast ecosystem of packages available through npm (node Package Manager).";
  ...
  test.test(
    "3.2 readFileAndCount, returns error if no word specified",
    (t, done) => {
      // write file tmp.txt with text fixture
      const filePath = path.join("tmp", "tmp.txt");
      const text =
      ...
      readFileAndCount(undefined, (err, count) => {
        assert.strictEqual(
          err.message,
          "No se ha especificado la palabra a buscar"
        );
        // remove tmp folder
        fs.rmSync("tmp", { recursive: true });
        done();
  ...
  test.test(
    "3.3 readFileAndCount, returns error if no path specified",
    (t, done) => {
      // call readFileAndCount
      readFileAndCount("word", (err, count) => {
        assert.strictEqual(
          err.message,
          "No se ha especificado el path del archivo"
        );
        done();
      });
    }
